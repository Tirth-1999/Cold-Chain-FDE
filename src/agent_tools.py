...
# Importing the langchain tool
from langchain_core.tools import tool

# Importing the openai embeddings
from langchain_openai import OpenAIEmbeddings

# Importing the pinecone vector store
...
EMBEDDINGS_MODEL_SETTING = os.getenv("EMBEDDING_MODE", "LOCAL").strip().upper()
...
def query_telemetry_db(sql_query: str) -> str:
    ...
    Latitude and Longitude are the GPS coordinates of the vehicle.
    Current_Temperature_C is the temperature of the vehicle.
    Cargo_Condition_Code is the condition of the cargo.
    Risk_Classification is the risk classification of the vehicle.
    Delay_Probability is the probability of delay of the vehicle.
    Port_Congestion_Level is the congestion level of the port.
    Route_Risk_Index is the risk index of the route.
    ...
    (2026-09-20 10:00:00, 33.77, -118.19, 20.0, 1, 0.5, 0.1, 0.2, 0.3)
    (2026-09-20 10:00:00, 33.77, -118.19, 20.0, 1, 0.5, 0.1, 0.2, 0.3)

    Example Error:
    SECURITY BLOCK: Only SELECT operations are authorized on this view.

    Example Error:
    Database Error: Invalid SQL query.
    """
    connection_string = (
...
def fetch_corridor_conditions(latitude: float, longitude: float) -> str:
    ...
    Example:
    fetch_corridor_conditions(33.77, -118.19)

    Example Output:
    --- LIVE CORRIDOR TELEMETRY ---
    Target GPS: 33.77, -118.19
    External Temp: 20.0°C | Wind Speed: 10.0 km/h
    Corridor Risk: High Transit Disruption (Congestion Index: 8.5/10)
    -------------------------------

    Example Error:
    Corridor API Communication Failure: Invalid latitude or longitude.

    Latitude and Longitude are the GPS coordinates of the vehicle.
    External Temp is the temperature of the vehicle.
    Wind Speed is the speed of the wind.
    Corridor Risk is the risk of the corridor.
    Congestion Index is the congestion index of the corridor.
    """
    try:
...
def search_compliance_sop(query: str) -> str:
    ...
    Example:
    search_compliance_sop("What is the regulatory threshold for temperature?")

    Example Output:
    --- COMPLIANCE SOP CONTEXT ---
    [Source: SOP_1.txt | Format: RAW]
    The regulatory threshold for temperature is 20°C.
    ------------------------------

    Example Error:
    Vector Store Retrieval Error: No matching compliance clauses found.

    Query is the query to search the compliance sop.
    Source is the source of the compliance sop.
    Format is the format of the compliance sop.
    Context is the context of the compliance sop.
    """
    try:
...
if __name__ == "__main__":
    ...
    print(search_compliance_sop.invoke("What are the Temperature Control & Spoilage Prevention?"))